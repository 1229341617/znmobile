    ...
    <div class="panel"  >
      ...
      <div class="panelBody" v-for="(item,index2) in itemvoList" v-show="'show'+index$1 === isOpen1">
        <ul class="entering">
          ...
          <li class="entering-itemsub">
            <div class="entering-left">
              <span class="entering-title">电脑型号:</span>
     ...
     showSheetVisible(){
      ...
      initIteminfo(){
        ...
        getBillDetail(params).then(res=>{
          Indicator.close();
          if (res.flag === "success"){
            that.computerData = res.billdata;
            if(that.computerData.length === 0){
              Toast("没有待审批数据");
              return;
            }
            ...
            that.writeoffList= res.billdata.computersub.writeoffvoList;
            that.itemvoList = res.billdata.computersub.itemVOList;
            that._imgURL = res.billdata.computersub.imgURL;
            that._attachmentList = res.billdata.computersub.attachmentList;

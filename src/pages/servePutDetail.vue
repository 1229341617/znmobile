<template>
  ...
  <div style="overflow: hidden">
    ...
    <ul class="entering">
      ...
      <li class="entering-item">
        <div class="entering-left">
          <span class="entering-title">费用使用说明：</span>
    ...
    <div class="panel"  >
      ...
      <div class="panelBody" v-for="(item,index2) in itemvoList" v-show="'show'+index$1 === isOpen1">
        <ul class="entering">
          ...
          <li class="entering-itemsub">
            <div class="entering-left">
              <span class="entering-title">允许报销最大金额:</span>
              <span class="entering-input">{{item.max_amount}}</span>
            </div>
          </li>
        </ul>
      </div>
    ...
    methods:{
      ...
      initIteminfo(){
        ...
        getBillDetail(params).then(res=>{
          Indicator.close();
          if (res.flag === "success"){
            that.servePutData = res.billdata;
            if(that.servePutData.length === 0){
              Toast("没有待审批数据");
              return;
            }
            ...
            that.itemvoList = res.billdata.servePutsub.itemVOList;
            that._imgURL = res.billdata.servePutsub.imgURL;
            that._attachmentList = res.billdata.servePutsub.attachmentList;

  ...
  <div style="overflow: hidden">
    ...
    <div class="panel"  >
      <div class="panelTitle" @click="openProject1(index$1)">
        <div class="titleLeft">销售宣传费用</div>
        <div class="titleRight">
      ...
      <div class="panelBody" v-for="(item,index2) in itemvoList" v-show="'show'+index$1 === isOpen1">
        <ul class="entering">
          ...
          <li class="entering-itemsub">
            <div class="entering-left">
              <span class="entering-title">发票号:</span>
              <span class="entering-input">{{item.fpdm}}</span>
            </div>
          </li>
          <li class="entering-itemsub" >
            <div class="entering-left">
              <span class="entering-title">税额:</span>
    ...
    methods:{
      ...
      initIteminfo(){
        ...
        getBillDetail(params).then(res=>{
          Indicator.close();
          if (res.flag === "success"){
            that.salesPropData = res.billdata;
            if(that.salesPropData.length === 0){
              Toast("没有待审批数据");
              return;
            }
            ...
            that.defitem20 = res.billdata.defitem20;
            that.zyx29 = res.billdata.zyx29;
            that.zyx5 = res.billdata.zyx5;
            that.writeoffList= res.billdata.salespropsub.writeoffvoList;
            that.itemvoList = res.billdata.salespropsub.itemVOList;
            that._imgURL = res.billdata.salespropsub.imgURL;
            that._attachmentList = res.billdata.salespropsub.attachmentList;

     ...
     showSheetVisible(){
      ...
      initIteminfo(){
        ...
        getBillDetail(params).then(res=>{
          ...
          //console.log(res);
          if (res.flag === "success"){
            that.borpsnData = res.billdata;
            if(that.borpsnData.length === 0){
              Toast("没有待审批数据");
              return;
            }
            ...
            that.itemvoList = res.billdata.brrowPsnsub.itemVOList;
            that._imgURL = res.billdata.brrowPsnsub.imgURL;
            that._attachmentList = res.billdata.brrowPsnsub.attachmentList;
            //console.log(that._attachmentList);
            if(that._attachmentList === null || that._attachmentList.length === 0){
              that.fuCount = 0;
            }else{
              that.fuCount = that._attachmentList.length;
            }
            //console.log(that._attachmentList.length);
            //that.fuCount = that._attachmentList.length;
